...
export const autoKick = {
    ...
    execute: async (message) => {
        ...
        if (message.content.includes('@everyone', 'here') && !message.member.permissions.has('MENTION_EVERYONE')) {
            const member = message.guild.members.cache.get(message.author.id);

            await message.delete().catch(console.error);

            const blacklist = ['csgo', 'cs:go', 'cs go', 'steam', 'stearn', 'kinfe', 'knife', 'skins', 'giveaway', 'free', 'nitro', 'discord', 'gift', 'first'];
